...
$messagebuf
  ...
  if($action eq "edit")
  {
    $buf2="<center>Для редактирования сценария заполните форму:</center>";
    $buf3="editplayfinish";
    ...
    if($playinfo[17] ne "")
    {
      ...
      $buf10 = "<tr>
<td colspan=5>Последние изменения вносил: $buf10 [$playinfo[4]]</td>
</tr>";
    }
    else
    {
    ...
    if(($Useridbuf ne $Userid)&($usertype ne "модераторы")&($usertype ne "администраторы"))
    {&netdostupa}
...
<tr><td>
<center><h2>Внимание!</h2></center>
Перед тем как добавлять сценарий, подумай, не нарушает ли он <a href=\"javascript:ShowHide('rules_open','rules_closed')\">правил Flash Mob'а</a><br>
<div id=rules_open style=\"display: none; z-index: 2;\"><br>
<b>1.</b> Flash Mob не должен носить политический, религиозный или коммерческий характер.<br>
<b>2.</b> Безопасность окружающих вас людей превыше всего.<br>Flash Mob не должен причинить вред и неудобства ни участникам, ни окружающим.<br>
<b>3.</b> Участники по возможности не должны общаться в период Flash Mob.<br>
<b>4.</b> Flash Mob должен начинаться и заканчиваться одинаково внезапно.<br>
</div><br>
</td></tr>
</table>
</td></tr>";
...
sub addplayfinish{
  ...
  if(param(predv)eq "Предварительный просмотр")
  {
    $title="Флешмоб в Краснодаре - Добавление нового сценария";
  ...
  if($ujebilo ne 1)
  {
    ...
    $textbuf="\[url=$site?play=$num\]\[size=5\]$playname\[/size\]\[/url\]\[br\]\[b\]Автор:\[/b\] \[url=$site?showuser=$Userid\]$Username\[/url\]\[br\]\[b\]Сценарий:\[/b\]\[br\]$message\[br\]\[br\]\[url=$site?play=$num\]Перейти к голосованию\[/url\]\[br\]\[url=$site?mode=forum&thread=$newthreadid\]Перейти на страницу обсуждений\[/url\]\[br\]\[url=$site\]Перейти на главную страницу сайта\[/url\]";

    ($buf,$to2)=&get_subscribe_mails(2);

    &send_subscribe($textbuf, "FMob: Добавлен новый сценарий \"$playname\"", "", "", $to2);
    &Inc_Col_Play;
...
sub editplayfinish {
  ...
  if(param(predv)eq "Предварительный просмотр")
  {
    $playname =~ s/"/&quot;/g;
    $redirectto="";
    $title="Флешмоб в Краснодаре - Редактирование сценария";
    &html;
    $action="edit";
    &addplay;
    &htmlend;
  ...
  if (($playinfo[3] ne $Userid)&($usertype ne "модераторы")&($usertype ne "администраторы"))
  {&netdostupa;}
...
sub showvoting{
  ...
  print "Всего сценариев - ",&colplay;

  if($shownotvoice ne 1){$pagebuf = "&page=$playpage"}else{$pagebuf = "&shownotvoice=1"}
  if($exp eq "1")
  {
    print " - <a href=\"$site?mode=voting$pagebuf\">Свернуть</a>\n";
  }
  else
  {
    print " - <a href=\"$site?mode=voting$pagebuf&exp=1\">Раскрыть</a>\n";
  }
  ...
  if($mode eq "voting"&$login eq 1)
  {
    if($LastLastTimeVisit ne "")
    {
      ...
      if($newplaycol>0)
      {
        ...
        print "<tr><td bgcolor=$tseriy align=center><b>С прошлого посещения ",&playstext($newplaycol),"</b></td></tr>";
    ...
    if($shownotvoice eq 1)
    {
      print "<b>[Сценарии, не оцененные вами]</b>";
    }
    else
    {
      print "<a href=$site?mode=voting&shownotvoice=1>Сценарии, не оцененные вами";
    }
    print "</td>\n<td nowrap align=right><a href=$site?mode=voting&action=add>Новый сценарий</a></td></tr>\n";
  }
  ...
  print "<td><b>Итог</b>";
  print "<td width=100\% align=center><b>Название</b>";
  print "<td><b>Обсуждение</b>";
  print "<td align=center><b>Автор</b>";
  print "</tr>";
  print "<tr><td>\n";
  print "</td></tr>\n";
  ...
  {
    print "<tr><td align=center colspan=6>";
    print "<input type=submit style=\"background-color: $tseriy;\" name=action value=Проголосовать>";
    print "</td></tr></form>\n"
  }
  print "</table>\n";
}

# Загружает идентификаторы сценариев в масив @indexofplay и параметры в хэши.
sub readindexofplay{
...
# Функция readmtn возвращает первый индификатор в файле.
sub readmtn{
...
sub noplayname{
  print "<br><br><center><b>Сценарий с таким именем уже присутствует в голосовании, либо был удалён из голосования ранее.</a></center></b>";
  &htmlend;
  exit;
}

# Функция colplay возвращает общее количество сценариев
sub colplay{
  $colplay=@indexofplay;
  return $colplay;
}
...
sub showmaxiplay{
  ...
  if($login eq 1&(($Useridbuf eq $Userid)|($usertype eq "модераторы")|($usertype eq "администраторы"))&$mode eq "voting")
  {
    print "<center><b><font size=\"1\">";
    print "[<a href=\"$site?mode=voting&action=delete&play=$readmtntemp\" class=\"adm\">Удалить</a>]\n";
    print "[<a href=\"$site?mode=voting&action=edit\&play=$readmtntemp\" class=\"adm\">Редактировать</a>]\n";
    if(($usertype eq "модераторы")|($usertype eq "администраторы"))
    {
      print "[<a href=\"$site?mode=voting&action=movetonext&play=$readmtntemp\" class=\"adm\">Назначить предстоящим мобом</a>]\n";
    }
    print "</font></b></center>";
  }

  print "<b>Сценарий:</b><br> $description\n";

  $buf4=&raznicadeys($date01, $nowtime);

  print "<br><br><b>Дата публикации:</b> $date01 ($buf4-й день в голосовании)\n";
  print "<br><br>\n";
...
# Выводим на каком месте данный сценарий и отмечаем значками тип
sub zazolovok_print{
  $image="";
...
sub get_voice_info{
  ...
  {
    $voice_plus2_users = "<b>0</b> мобберов</td></tr>";
  }
  ...
  {
    $voice_plus1_users = "<b>0</b> мобберов</td></tr>";
  }
  ...
  {
    $voice_nul_users = "<b>0</b> мобберов</td></tr>";
  }
  ...
  {
    $voice_minus1_users = "<b>0</b> мобберов</td></tr>";
  }
  ...
  {
    $voice_minus2_users = "<b>0</b> мобберов</td></tr>";
  }
...
sub itogo_print{
  ...
  print "<tr bgcolor=$seriy><td colspan=5><b>Голосование</b></td></tr>\n";

  print "<tr bgcolor=$seriy><td nowrap width=34 height=24 align=center>$voice_plus2_buf</td><td width=90%>$voice_plus2_buf2Отличный сценарий, очень хочу поучаствовать</td><td nowrap>$voice_plus2_users\n";
  print "<tr bgcolor=$seriy><td nowrap width=34 height=24 align=center>$voice_plus1_buf</td><td>$voice_plus1_buf2Хороший сценарий, но есть и лучше</td><td nowrap>$voice_plus1_users\n";
  print "<tr bgcolor=$seriy><td nowrap width=34 height=24 align=center>$voice_nul_buf</td><td>$voice_nul_buf2Не могу определиться</td><td nowrap>$voice_nul_users\n";
  print "<tr bgcolor=$seriy><td nowrap width=34 height=24 align=center>$voice_minus1_buf</td><td>$voice_minus1_buf2Против</td><td nowrap>$voice_minus1_users\n";
  print "<tr bgcolor=$seriy><td nowrap width=34 height=24 align=center>$voice_minus2_buf</td><td>$voice_minus2_buf2Никуда не годится</td><td nowrap>$voice_minus2_users\n";
...
sub deleteplay {
  if(param(deleteans) eq "")
  {
    ...
    if(!(($playinfo[3] eq $Userid)|($usertype eq "модераторы")|($usertype eq "администраторы")))
    {
      &netdostupa;
    }
    ...
    print "Удаление $plays сценария \"<b>$playinfo[5]</b>\".<br>\n";
    print "Вы действительно ходите удалить сценарий из голосования?\n";
    print "</td></tr>\n";
    print "<tr align=center><td>\n";
    print "<input type=submit style=\"background-color: $tseriy;\" name=deleteans value=\"Да \">\n";
    print "<input type=submit style=\"background-color: $tseriy;\" name=deleteans value=\"Нет\">\n";
    print "</td></tr>\n";
    print "</form>\n";
    print "</table>\n";
  }
  if(param(deleteans) eq "Нет")
  {
    $redirectto = param(REFERER);
    &html;
    exit;
  }
  if(param(deleteans) eq "Да ")
  {
    open(playinfo, "<plays/$play.txt");
    flock(playinfo, 1);
    @playinfo=<playinfo>;
